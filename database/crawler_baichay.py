...
class BaiChayCrawler:
    ...
    def extract_detail_info(self, url: str, category_type: str) -> Optional[Dict]:
        ...
        try:
            ...
            for img in images:
                src = img.get('src', '')
                if src and ('wp-content' in src or 'upload' in src) and not (
                        'logo' in src.lower() or 'icon' in src.lower()):
    ...
    def crawl_category(self, category_type: str, max_items: int = None, max_pages: int = 10,
                       use_safe_method: bool = True) -> List[Dict]:
        """
        Crawl một category
        Args:
    ...
    def crawl_all_categories(self, max_items_per_category: int = None, max_pages_per_category: int = 10,
                             use_safe_method: bool = True) -> Dict[str, List[Dict]]:
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n🧪 Testing with 'diem-den' category (safe method, all pages)...")
        test_data = crawler.crawl_category(
            "diem-den",
            max_items=15,  # Giới hạn 15 items để test nhanh
            max_pages=10,
        ...
        if test_data:
            ...
            print(f"  Images: {len(json.loads(sample['image_urls']))} images")
            print(f"  Description length: {len(sample['description'])} chars")
            print(f"  Vector dim: {len(sample['description_vector'])}")

        # Uncomment để crawl tất cả categories (TẤT CẢ các trang)
        # print("\n🚀 Crawling all categories (all pages)...")
        # all_data = crawler.crawl_all_categories(
